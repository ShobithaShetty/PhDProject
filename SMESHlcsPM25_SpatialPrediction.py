"""
This python code generates daily PM2.5 spatial prediction for a given month and year, based on S-MESH (LCS-as-predictor version) ML framework
...
Outputs: Daily tif files with PM2.5 prediction over central europe
...
xgbmodelpathFA = "/xnilu_wrk/users/sshe/LCSProject/models/TestIni/CU_LCS_XGBModelCAMSForAODnulls.model"
xgbmodelpathF = "/xnilu_wrk/users/sshe/LCSProject/models/Final/CU_LCS_XGBModelCAMSFornulls.model"
xgbmodelpathC = "/xnilu_wrk/users/sshe/LCSProject/models/Final/CU_LCS_XGBModelComP.model"
...
xgbpredictFile = '/xnilu_wrk/users/sshe/LCSProject/datasets/Europe/Predictions/Outputs/PM25/'

ext = 'tif'

### Defining the study extent. Thus wull help crop the input rasters/nc files which are in European domain
minx = 2.24
maxx = 24.21
miny = 45.58
maxy = 55.01
...
def clip_raster(ref):
    ...
    #ref = ref.rio.clip_box(minx=9.392, miny=51.175, maxx=10.725, maxy=52.742)
    ref = ref.rio.clip_box(minx=minx, miny=miny, maxx=maxx, maxy=maxy)
    return ref
...
def predictionToTif(mpredictions, refTif, fpath, scenario):
    ...
    if not os.path.exists(outputf + fpath + year+'/'+month+'/'):
        os.makedirs(outputf + fpath + year+'/'+month+'/')

    outfilename = outputf + fpath + year+'/'+month+'/'+ 'EU_lcsPM2P5'+ '_' + when+ '_' + scenario  + '.' + ext
    print("Masking done")
...
def read_data(when):
    ...
    fpath = "/xnilu_wrk/users/sshe/LCSProject/datasets/WeighAvg/SpatialOutputs/D2/"+year+"/"+month+"/D2IDW_lcsPM2P5_"+when+".tif"
    sev = ri.open_rasterio(fpath)
    sev = clip_raster(sev)
    sevD = sev.values
    sevD = sevD.flatten()
    df['D2C_WeighAvgPM25'] = sevD

    fpath = "/xnilu_wrk/users/sshe/LCSProject/datasets/Distances/2122_NearestDisttoLCS_CUgrid.tif"
    sev = ri.open_rasterio(fpath)
    sev = clip_raster(sev)
    sevD = sev.values
    sevD = sevD.flatten()
    df['NearLCSDist_m'] = sevD
...
def preprocess(df):
    ...
    fcolumns = [
        ...
        'D2C_WeighAvgPM25','NearLCSDist_m',
    ...
    powTrans = ['sp', 'DEM', 'DEM_Std5km',
                'Aerosol_Optical_Thickness_550_Land_Best_Estimate', 'popDen','absorbing_aerosol_index']  #'absorbing_aerosol_index',
...
def performML(xdf,refTif):
    ...
    print("Prediction")
    print("What are the columns used in prediction here", xdf.columns)
    pred_map = xgb.DMatrix(xdf.values, feature_names=list(xdf.columns))
    ...
    # 2ND MODEL
    xxdf = xdf.drop(
        columns=['Aerosol_Optical_Thickness_550_Land_Best_Estimate', 'Angstrom_Exponent_Land_Best_Estimate','D2C_WeighAvgPM25','NearLCSDist_m'])
    predf_map = xgb.DMatrix(xxdf.values,feature_names=list(xxdf.columns))
    ...
    tf = pd.concat([xdf['Aerosol_Optical_Thickness_550_Land_Best_Estimate'], pd.DataFrame({'ForAODP': FApredictions}),
                    pd.DataFrame({'ForP': Fpredictions}), xdf['pm2p5_conc'],xdf['NearLCSDist_m']], axis=1)
    print("Combined dataframe okay?", tf.head())
    cpred_map = xgb.DMatrix(tf.values,feature_names=list(tf.columns))
    Cpredictions = xgbC.predict(cpred_map)
    print("Third Prediction", Cpredictions)



    predictionToTif(FApredictions, refTif,'Famodel/', 'mFA')
    predictionToTif(Fpredictions, refTif, 'Fmodel/','mF')
...
powTrans = ['sp', 'DEM', 'DEM_Std5km',
            'Aerosol_Optical_Thickness_550_Land_Best_Estimate', 'popDen','absorbing_aerosol_index'] 

## Generate power transform from training data

modelData = pd.read_csv('/xnilu_wrk/users/sshe/LCSProject/datasets/InputParams/Test/2021_2022nonTransformedData_withConv_CU.csv',
                        sep=';')
